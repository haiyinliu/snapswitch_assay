...
`data.csv` was directly exported from Flowjo with these columns: AF488/Cy5/mScarlet gMFI, GROUPNAME (name starts with a number between 1-4, this is the `group_ID`), and TUBENAME (name starts with a `sample_ID` and replicate suffix eg. `LNP1_001`).

#### Sample key files

The script looks for key files in a `parameters` folder. It should contain

`groups.csv` links the `group_ID` to information about cells (`WT` or `LSA`), sensor (`SNAP` or `Cy5`).
...
exp_title <- "241024 DC1940 panel new #2"        # date and name of the experiment
...
lipid_properties <- read_csv("parameters/lipid_properties.csv")

# makes sure that the sample_names are being ordered by their sample_ID (eg "LNP1", "LNP2" etc)
...
This code performs data cleaning, subtracts the background MFIs from untreated cells and calculates the endosomal escape values. Following values will be returned:

-   raw gMFIs (on LSA cells)
    -   AF488 gMFI
    -   SNAP gMFI
    -   mScarlet gMFI
    -   Cy5-LNP gMFI
-   adjusted endosomal escape (SNAP signal normalised to AF488 on WT cells for each LNP (formulation + Ab combination) as its own formulation
    -   absolute (WT bg subtracted)
    -   relative (additionally divided by AF488 signal)
    -   % escape (relative to "always on" Cy5-LNP) 
...
facs <- facs_unclean[,-1] %>%
  ...
  filter(!grepl('mystery|Unstained|LNP17', TUBENAME)) %>% # removes "mystery" and Aurora "Unstained" tubes
  filter(!TUBENAME == "-")                                          # removes mean and SD rows
colnames(facs) = c("AF488_raw","Cy5_raw","mScarlet_raw","GROUPNAME","TUBENAME","viability")
...
facs <- facs %>% 
  ...
  # set the order in which the samples appear on graphs
  mutate(sample_name = factor(sample_name, levels = sample_order))
```

```{r analysis, echo=FALSE}
####--------CALCULATION--------####
# for each sample, test if values are significantly different from the unstained samples
facs_significant <- facs %>%
  group_by(cells, group_ID, sample_ID) %>%
  mutate(
    significant_AF488 = t.test(AF488_raw, facs$AF488_raw[facs$sample_ID == untreated_ID])$p.value < 0.05,
    significant_mScarlet = t.test(mScarlet_raw, facs$mScarlet_raw[facs$sample_ID == untreated_ID])$p.value < 0.05,
    significant_Cy5 = t.test(Cy5_raw, facs$Cy5_raw[facs$sample_ID == untreated_ID])$p.value < 0.05,
  )
...
facs_raw <- facs_significant %>%
  ...
  mutate(
    ...
    Cy5 = if_else(significant_Cy5, Cy5_raw - mean(Cy5_raw[sample_ID == untreated_ID]), 0),
...
# TODO need to check why this doesn't work
# check if SNAP values are significantly different between LSA and WT cells
# facs_raw <- facs_raw %>%
#   group_by(sample_ID) %>%
#   mutate(
#     significant_SNAP = case_when(
#       sensor == "SNAP" ~ t.test(Cy5[cells == "WT"], Cy5[cells == "LSA"])$p.value < 0.05,
#       sensor == "Cy5" ~ NA
#     )
#   ) %>%
#   ungroup()
...
# final calculations to normalise Cy5 to 488 MFI for both SNAP-LNP and Cy5-LNP.
facs_norm <- facs_raw %>%

  # treat EACH formulation/sample combo as its own "formulation"
  group_by(sensor,formulation,sample_ID) %>%   

  # calculations
  mutate(norm_factor = AF488 / mean(AF488[cells == "WT"]),
         adj_Cy5 = Cy5 / norm_factor,
         adj_Cy5_abs = ifelse(sensor == "SNAP", adj_Cy5 - mean(adj_Cy5[cells == "WT"]), adj_Cy5), 
         # for SNAP samples, subtracts WT signal, don't subtract WT from Cy5)
         adj_Cy5_rel = adj_Cy5_abs / mean(AF488[cells == "WT"]), # divide by WT AF488 signal
         express_per_assoc = mScarlet / AF488,
         express_per_escape = mScarlet / adj_Cy5_abs) %>% 
  ungroup()
...
results_SNAP <- results_SNAP %>%
  left_join(lipid_properties, by = c("sample_name" = "lipid_name"))
...
bar_chart <- function(df,column,colour) {
  df %>% 
    ...
    summarise(mean = mean(!!sym(column)),
              sd = sd(!!sym(column))) %>%
    ggplot(aes(x=reorder(sample_name, -mean), y=mean, fill = factor(formulation, formulation_order))) +
...
for(i in 1:length(datarange)){plot_list[[i]] <- bar_chart(results_SNAP, datarange[i], colour_range[i])}
...
ggsave(paste0("output/", exp_title,".png"), plot = all_plots, width = 9, height = 6, units = "in")
all_plots
```
...
# generate plots for each variable in the datarange and match with the colour range
for(i in 1:length(datarange)){plot_list_pka[[i]] <- plots_pka(results_2D, datarange[i], colour_range[i])}
...
# save and display plots
ggsave(paste0("output/", exp_title,"_pKa.png"), plot = all_plots_pka, width = 10, height = 8, units = "in")
all_plots_pka
...
# 
# # generate plots for each variable in the datarange and match with the colour range
# for(i in 1:length(datarange)){plot_list_2D[[i]] <- plots_pkavtails(results_2D, datarange[i], colour_range_2D[i])}
# 
# # combine all plots in the list into an array (patchwork package)
...
# define colour of the plots for each variable
colour_range_raw <- c("darkseagreen",
                   "skyblue2",
                   "indianred",
                   "grey68")
...
# generate plots for each variable in the datarange and match with the colour range
for(i in 1:length(datarange_raw)){SNAP_QC[[i]] <- QC_bar_chart(results_raw[["SNAP"]], datarange_raw[i], colour_range_raw[i])}
for(i in 1:length(datarange_raw)){Cy5_QC[[i]] <- QC_bar_chart(results_raw[["Cy5"]], datarange_raw[i], colour_range_raw[i])}
...
write.xlsx(results_list, file = paste0("output/", exp_title,".xlsx"))

# COMBINED ANALYSIS FILE
# write a csv of the processed data to feed into the SNAPSwitch_combined analysis 
write.csv(results_SNAP, file = paste0("output/", exp_title,"_SNAP.csv"))
...
write.csv(results_SNAP_pka, file = paste0("output/", exp_title,"_pKa.csv"))
